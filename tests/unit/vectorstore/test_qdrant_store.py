...
# ---------------------------------------------------------------------------
# get_all_chunks_by_source / _resolve_exact_source
# ---------------------------------------------------------------------------
...
class TestGetAllChunksBySource:

    def test_requires_user_id(self):
        store = _make_store()
        with pytest.raises(ValueError, match="TENANT_ISOLATION_VIOLATION"):
            store.get_all_chunks_by_source("apple_10k.pdf", user_id="")

    def test_empty_source_returns_empty_without_scroll(self):
        store = _make_store()
        store._collection_cache = {"text_collection"}
        result = store.get_all_chunks_by_source("", user_id="u1")
        assert result == []
    ...
    def test_no_matching_source_returns_empty(self):
        ...
        result = store.get_all_chunks_by_source("nonexistent.pdf", user_id="u1")
        assert result == []

    def test_resolves_hashed_filename_and_sorts_by_page(self):
        """Stored `source` carries an ingestion-time hash prefix
        (e.g. "abc123_apple_10k.pdf"); a human-typed filename must still
        resolve via the same case-insensitive substring convention
        hybrid_retriever.py uses for its own source filter."""
        store = _make_store()
        store._collection_cache = {"text_collection"}

        resolve_points = [_point({"source": "abc123_apple_10k.pdf"})]
        fetch_points = [
            _point(
                {"text": "second", "source": "abc123_apple_10k.pdf", "page": 2, "chunk_id": "c2"}
            ),
            _point(
                {"text": "first", "source": "abc123_apple_10k.pdf", "page": 1, "chunk_id": "c1"}
            ),
        ]
        store.client.scroll.side_effect = [
            (resolve_points, None),  # _resolve_exact_source scan
            (fetch_points, None),  # main fetch
        ]

        result = store.get_all_chunks_by_source("apple_10k.pdf", user_id="u1")

        assert [r["text"] for r in result] == ["first", "second"]

    def test_chunk_without_text_is_skipped(self):
        store = _make_store()
        store._collection_cache = {"text_collection"}
        resolve_points = [_point({"source": "apple_10k.pdf"})]
        fetch_points = [
            _point({"source": "apple_10k.pdf", "page": 1}),  # no "text" — skipped
            _point({"text": "kept", "source": "apple_10k.pdf", "page": 2}),
        ]
        store.client.scroll.side_effect = [(resolve_points, None), (fetch_points, None)]

        result = store.get_all_chunks_by_source("apple_10k.pdf", user_id="u1")

        assert [r["text"] for r in result] == ["kept"]
