...
# g6e.xlarge hit InsufficientInstanceCapacity in us-east-1a in this account
# (confirmed live during the account-migration bring-up, 2026-09-08; AWS's own
# error suggested 1b/1c/1d). EBS volumes are AZ-locked, so production's model
# volume had to move with it. Uptime Kuma (t4g.micro, no capacity issue) and
# any future staging box stay on the original var.availability_zone subnet —
# only production's placement moved.
