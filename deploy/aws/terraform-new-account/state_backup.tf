...
#   aws s3 cp deploy/aws/terraform-new-account/terraform.tfstate \
#     s3://magik-terraform-state-<this account's ID>/terraform.tfstate
# Bucket name is derived from the current account ID (not hardcoded) — S3
# bucket names are globally unique, so a literal copy of the old account's
# bucket name here would collide with the old account's own bucket.
resource "aws_s3_bucket" "terraform_state_backup" {
  bucket = "magik-terraform-state-${data.aws_caller_identity.current.account_id}"
