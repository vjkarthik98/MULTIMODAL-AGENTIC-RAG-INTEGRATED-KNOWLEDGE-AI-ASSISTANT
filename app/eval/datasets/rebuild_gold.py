...
MODALITY_FILES = {
    "txt": "text_gold.jsonl", "pdf": "pdf_gold.jsonl", "docx": "docx_gold.jsonl",
    "xlsx": "xlsx_gold.jsonl", "image": "image_gold.jsonl", "audio": "audio_gold.jsonl",
    "video": "video_gold.jsonl", "e2e": "e2e_gold.jsonl", "routing": "routing_gold.jsonl",
}
...
_RATING = re.compile(r"\b(?:Aaa|Aa[123]|A[123]|Baa[123]|Ba[123]|B[123]|Caa[123]|Ca|"
...
CITE_LOCATOR_TYPE = {
    "txt": "section", "pdf": "page", "docx": "section", "xlsx": "sheet",
    "image": "image_title", "audio": "timestamp", "video": "timestamp",
}
...
def expected_citation(row: dict, modality: str, is_websearch: bool, behavior: str) -> dict:
    ...
    if modality == "xlsx":
        ...
        if frame_cap and has_transcript:
            ...
            locator_type = "timestamp"
        return {"source": sf, "locator_type": locator_type, "locator": None,
...
def classify(row: dict) -> tuple[str, str]:
    ...
    # question_type
    qualitative = any(k in q for k in (
        "what reasons", "describe", "what risk", "competitive pressure", "what did",
        "pillars", "explain", "summariz", "thesis", "how does .* get applied",
    ))
    comparative = ("comparative-analysis" in tags or any(
        k in q for k in ("compare", "versus", " vs ", "how did", "how does", "compared")))
    numeric = ("financial-reasoning" in tags or "ratio-analysis" in tags or any(
        k in q for k in ("growth rate", "percentage", "as a percent", "how much did",
                         "basis point", "what was its year-over-year")))
    temporal = any(k in q for k in ("as of", "end of 20", "over the 12", "over the three",
    ...
    # difficulty
    if nums >= 3 or {"ratio-analysis", "financial-reasoning"} & tags and qtype in ("numeric-reasoning", "comparative"):
        difficulty = "hard"
...
def adversarial_row(modality: str, n: int, kind: str, query: str, reference: str,
                    chunk_ids: list[str], must_include: list[str],
                    must_not: list[str]) -> dict:
    cite = expected_citation(
        {"source_file": SRC.get(modality), "query": query, "reference_answer": reference},
        modality, is_websearch=False, behavior="answer",
    )
    return {
...
ADVERSARIALS: dict[str, list[tuple]] = {
    "txt": [
        ("prompt-injection",
         ...
         ["fomc_dec2024.txt::chunk_1", "fomc_dec2024.txt::chunk_5"], ["25 basis points", "4.25-4.50"], ["OVERRIDE"]),
        ("false-premise",
         ...
         ["fomc_dec2024.txt::chunk_1", "fomc_dec2024.txt::chunk_5"], ["25 basis points", "4.25-4.50"], []),
    ],
    "pdf": [
        ("prompt-injection",
         ...
         ["apple_10k.pdf::chunk_725"], ["391,035"], ["APPROVED"]),
        ("false-premise",
         ...
         ["apple_10k.pdf::chunk_725"], ["391,035", "383,285"], []),
    ],
    "docx": [
        ("prompt-injection",
         ...
         ["apple_investment_research_report.docx::chunk_0"], ["$245.00"], ["BUY AT ANY PRICE"]),
        ("false-premise",
         ...
         ["apple_investment_research_report.docx::chunk_0"], ["$245.00"], []),
    ],
    "xlsx": [
        ("prompt-injection",
         ...
         ["ctryprem.xlsx::chunk_6"], ["7.075%", "Baa3"], ["999"]),
        ("false-premise",
    ...
    "image": [
        ("prompt-injection",
         ...
         ["aapl-20240928_g2.jpg::chunk_0"], [], ["N/A"]),
        ("false-premise",
    ...
    "audio": [
        ("prompt-injection",
         ...
         ["FOMC Press Conference September 18, 2024.mp3::chunk_3"], ["50 basis points", "4.75-5.00"], ["MUTED"]),
        ("false-premise",
         ...
         ["FOMC Press Conference September 18, 2024.mp3::chunk_3"], ["50 basis points", "4.75-5.00"], []),
    ],
    "video": [
        ("prompt-injection",
         ...
         ["Q4 2025 Earnings Call.mp4::chunk_1"], ["$102.5 billion", "8 percent"], ["CONFIDENTIAL"]),
        ("false-premise",
         ...
         ["Q4 2025 Earnings Call.mp4::chunk_1"], ["$102.5 billion", "8 percent"], []),
    ],
}
...
def e2e_row(n: int, spec: tuple) -> dict:
    ...
    return {
        "id": f"e2e-{n:04d}", "modality": "e2e", "source_file": None,
        "difficulty": "hard", "question_type": qtype, "expected_behavior": "answer",
        "query": query, "relevant_doc_ids": doc_ids, "relevant_chunk_ids": chunk_ids,
        "distractor_doc_ids": [], "reference_answer": ref, "must_include_facts": facts,
        "must_not_include": [],
        "expected_citation": {"source": doc_ids, "locator_type": "multi", "locator": None},
        "rubric_id": None, "expected_route": "rag",
        "tags": ["cross-corpus", "e2e-pipeline"], "human_verified": False,
        "verified_by": None, "added_by": AUTHOR, "added_at": TODAY,
    }


E2E_SEARCH = {
    "id": "e2e-0004", "modality": "e2e", "source_file": None,
    "difficulty": "medium", "question_type": "temporal", "expected_behavior": "answer",
    "query": "What is the current stock price of Apple?",
    "relevant_doc_ids": [], "relevant_chunk_ids": [], "distractor_doc_ids": [],
    "reference_answer": "SEARCH_REQUIRED: real-time market data not in the KB; correct route is web search.",
    "must_include_facts": [], "must_not_include": [],
    "expected_citation": {"source": None, "locator_type": "web", "locator": None},
    "rubric_id": None, "expected_route": "search",
    "tags": ["real-time", "routing-test", "e2e-pipeline"], "human_verified": False,
    "verified_by": None, "added_by": AUTHOR, "added_at": TODAY,
}
...
EXTRA_ROUTING = [
    ("route-0013", "What was the FOMC's target range for the federal funds rate in December 2024?", "rag", ["fact-extraction", "routing-eval"]),
    ...
    ("route-0016", "What did I ask you about Apple earlier in this session?", "memory", ["memory-recall", "routing-eval"]),
    ("route-0017", "Summarize the risk factors in Apple's 10-K and cross-check them against the latest analyst commentary online.", "hybrid", ["hybrid-rag-web", "routing-eval"]),
    ("route-0018", "Define what a country risk premium is.", "rag", ["general-knowledge", "routing-eval"]),
    ("route-0019", "What is Apple's Services revenue for fiscal 2024 per the 10-K?", "rag", ["fact-extraction", "routing-eval"]),
    ("route-0020", "What's the weather in Cupertino today?", "hybrid", ["real-time", "off-domain", "routing-eval"]),
]


def routing_row(rid: str, query: str, route: str, tags: list[str]) -> dict:
    return {
        "id": rid, "modality": "routing", "source_file": None,
        "difficulty": "medium", "question_type": "routing", "expected_behavior": "answer",
        "query": query, "relevant_doc_ids": [], "relevant_chunk_ids": [],
        "distractor_doc_ids": [], "reference_answer": route, "must_include_facts": [],
        "must_not_include": [], "expected_citation": {"source": None, "locator_type": "none", "locator": None},
        "rubric_id": None, "expected_route": route,
        "tags": tags, "human_verified": False, "verified_by": None,
        "added_by": AUTHOR, "added_at": TODAY,
    }
...
def read_content_rows(path: Path) -> list[dict]:
    ...
    return [
        ...
        if not _BEHAVIORAL_ID.search(r.get("id", ""))
        and r.get("id") not in _EXTRA_ROUTING_IDS
    ]
...
def build(dry: bool = False) -> None:
    ...
    routing_new = [routing_row(r["id"], r["query"], r.get("expected_route", r.get("reference_answer", "rag")), r.get("tags", []))
                   for r in routing_old]
    routing_new += [routing_row(*spec) for spec in EXTRA_ROUTING]
    summary["routing"] = {"total": len(routing_new), "rewritten": len(routing_old), "added": len(EXTRA_ROUTING)}
    if not dry:
