...
def _print_gpu_state(label: str = "") -> None:
    if torch.cuda.is_available():
        ...
        print(
            f"[GPU{' '+label if label else ''}] free={free/1024**3:.1f}GB / total={total/1024**3:.1f}GB"
        )
...
def evaluate_web(row: dict[str, Any]) -> dict[str, Any]:
    ...
    web_grounded = (
        1 if any(isinstance(s, dict) and s.get("modality") == "web" for s in sources) else 0
    )
