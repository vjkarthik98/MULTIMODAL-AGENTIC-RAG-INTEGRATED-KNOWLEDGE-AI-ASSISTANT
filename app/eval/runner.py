...
class EvalRunner:
        ...
        for suite in suites:
            ...
            for name, m in suite_result.metrics.items():
                if not math.isnan(m.value):
                    print(f"  {name:40s} = {m.value:.4f}  (n={m.n}) {m.notes[:60] if m.notes else ''}")
                else:
    ...
    @staticmethod
    def _find_threshold(thresholds: Dict[str, Any], metric_name: str) -> Optional[Dict]:
            ...
            if isinstance(section_val, dict):
                ...
                for v2 in section_val.values():
                    if isinstance(v2, dict) and metric_name in v2:
                        ...
                        if isinstance(candidate, dict) and ("min" in candidate or "max" in candidate):
                            return candidate
        return None
