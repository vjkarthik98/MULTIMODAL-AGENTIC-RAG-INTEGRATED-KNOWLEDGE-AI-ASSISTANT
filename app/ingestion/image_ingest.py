...
# Run: pytest app/ingestion/image_ingest.py -v
# ============================================================
...
class TestImageIngest:
    ...
    def _make_image(self, tmp_path, name: str = "test.jpg", size=(200, 200), color=(100, 150, 200)) -> Path:
        path = tmp_path / name
    ...
    async def test_happy_path_returns_chunks_and_metadata(self, tmp_path):
        ...
        with patch("app.ingestion.image_ingest._generate_caption", return_value="A bright blue test image with geometric shapes"):
            with patch("app.ingestion.image_ingest._extract_ocr", return_value=""):
                docs = await async_ingest(str(path), "s1")
        assert docs
        assert docs[0].metadata is not None
        assert docs[0].modality == "image"
    ...
    @pytest.mark.asyncio
    async def test_corrupt_file_raises(self, tmp_path):
        path = tmp_path / "corrupt.jpg"
        path.write_bytes(b"\xff\xd8\xff\x00" + b"\x00" * 100)
    ...
    async def test_file_too_large_raises(self, tmp_path):
        path = self._make_image(tmp_path)
        with patch("app.core.config.settings.MAX_FILE_SIZE_IMAGE", 1):
            with pytest.raises(FileTooLargeError):
                await async_ingest(str(path), "s1")

    @pytest.mark.asyncio
    async def test_zero_dimension_rejected(self, tmp_path):
        path = self._make_image(tmp_path, size=(200, 200))
        with patch("app.ingestion.image_ingest._validate_dimensions", side_effect=ValueError("INVALID_IMAGE_DIMENSIONS")):
            with pytest.raises(ValueError, match="INVALID_IMAGE_DIMENSIONS"):
                await async_ingest(str(path), "s1")

    @pytest.mark.asyncio
    async def test_ocr_extracts_text_from_image(self, tmp_path):
        path = self._make_image(tmp_path)
        with patch("app.ingestion.image_ingest._extract_ocr", return_value="Invoice Total 1234 USD"):
            with patch("app.ingestion.image_ingest._generate_caption", return_value="A document with invoice text and numbers"):
                docs = await async_ingest(str(path), "s1")
        ocr_docs = [d for d in docs if d.subtype == "ocr"]
        assert ocr_docs
        assert "Invoice" in ocr_docs[0].text

    @pytest.mark.asyncio
    async def test_phash_near_duplicate_detected(self, tmp_path):
        path = self._make_image(tmp_path, color=(100, 100, 100))
    ...
    @pytest.mark.asyncio
    async def test_thumbnail_generated(self, tmp_path):
        path = self._make_image(tmp_path)
        with patch("app.ingestion.image_ingest._generate_caption", return_value="A bright blue test image with shapes"):
            with patch("app.ingestion.image_ingest._extract_ocr", return_value=""):
                docs = await async_ingest(str(path), "s1")
        assert any(d.structure.get("thumbnail_path") for d in docs)

    @pytest.mark.asyncio
    async def test_exif_gps_anonymized_in_privacy_mode(self, tmp_path):
        path = self._make_image(tmp_path)
        with patch("app.core.config.settings.IMAGE_PRIVACY_MODE", True):
            exif = _extract_exif(path)
        assert "gps" not in exif or exif.get("gps_stripped") is True
    ...
    def test_solid_color_flagged(self):
        img = Image.new("RGB", (200, 200), color=(255, 255, 255))
        assert _is_solid_color(img) is True

    def test_watermark_detection(self):
        assert _detect_watermark("CONFIDENTIAL document") is True
        assert _detect_watermark("Normal photo of a cat") is False

    def test_aspect_ratio_extreme_flagged(self):
        result = _check_aspect_ratio(2000, 10)
        assert result is not None
        assert "EXTREME" in result

    def test_caption_clean_strips_weak_prefix(self):
        raw = "a photo of a red car parked on the street"
    ...
    @pytest.mark.asyncio
    async def test_password_protected(self, tmp_path):
        # IMAGES DONT HAVE PASSWORDS BUT VERIFY UNSUPPORTED MIME RAISES
        path = tmp_path / "file.xyz"
        path.write_bytes(b"\x00\x01\x02\x03")
        with pytest.raises((UnsupportedMimeError, ValueError)):
            await async_ingest(str(path), "s1")