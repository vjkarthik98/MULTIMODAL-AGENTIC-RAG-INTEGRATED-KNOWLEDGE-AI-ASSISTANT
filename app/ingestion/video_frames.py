...
# CLEANUP HELPER — CALLED BY video_ingest.py AFTER PROCESSING

def cleanup_frame_temp_dir(temp_dir_path: str) -> None:
    """Remove entire temp frame directory — must be called in finally block."""
    try:
        shutil.rmtree(temp_dir_path, ignore_errors=True)
        logger.info(event="frame_temp_dir_cleaned", path=temp_dir_path)
    except Exception as exc:
        logger.warning(event="frame_temp_dir_cleanup_failed", error=str(exc))
...
class TestExtractFrames:
    ...
    def test_session_id_required_raises(self):
        with pytest.raises(ValueError, match="SESSION_ID_REQUIRED"):
            extract_frames("/tmp/test.mp4", 2, "")

    def test_missing_file_raises(self):
        with pytest.raises(FileNotFoundError, match="VIDEO_NOT_FOUND"):
            extract_frames("/nonexistent/video.mp4", 2, "sess-001")

    def test_empty_file_raises(self, tmp_path):
        empty = tmp_path / "empty.mp4"
        empty.write_bytes(b"")
        with pytest.raises(ValueError, match="EMPTY_VIDEO_FILE"):
            extract_frames(str(empty), 2, "sess-001")

    def test_disk_space_guard_raises_on_low_space(self, monkeypatch):
        monkeypatch.setattr(
            "shutil.disk_usage",
            lambda _: MagicMock(free=1024 * 1024),
        )
        with pytest.raises(DiskSpaceError):
            _check_disk_space("/tmp")

    def test_probe_video_returns_defaults_on_failure(self):
        probe = _probe_video("/nonexistent/file.mp4")
        assert "fps" in probe
        assert "hdr" in probe
        assert "interlaced" in probe
        assert probe["fps"] >= 0
    ...
    def test_brightness_mean_range(self):
        if cv2 is None:
            pytest.skip("cv2 not available")
        white = np.full((100, 100, 3), 255, dtype=np.uint8)
        b     = _brightness_mean(white)
        assert b > 200.0

    def test_is_too_dark_true_for_black_frame(self):
        assert _is_too_dark(0.0) is True

    def test_is_too_dark_false_for_bright_frame(self):
        assert _is_too_dark(200.0) is False

    def test_phash_distance_same_hash(self):
        if not IMAGEHASH_AVAILABLE:
            pytest.skip("imagehash not available")
        import imagehash as ih
        h = str(ih.average_hash(PILImage.new("RGB", (64, 64), 128)))
        assert _phash_distance(h, h) == 0

    def test_compute_phash_returns_none_for_missing_file(self):
        result = _compute_phash("/nonexistent/image.jpg")
        assert result is None

    def test_frame_metadata_to_dict_keys(self):
        meta = FrameMetadata(
            frame_index=0, timestamp_start=0.0, timestamp_end=2.0,
            path="/tmp/f.jpg", frame_width=640, frame_height=360,
            fps=25.0, video_duration=60.0, video_id="test.mp4",
            blur_score=0.9, brightness_mean=128.0, phash="abc123",
            is_scene_boundary=True, scene_index=0, shot_type="wide",
            hdr_detected=False, interlaced=False, aspect_ratio=1.78,
            extraction_method="pyscenedetect", session_id="sess-001",
        )
        d = meta.to_dict()
        required_keys = [
            "frame_index", "timestamp_start", "timestamp_end", "path",
            "frame_width", "frame_height", "fps", "video_duration",
            "video_id", "blur_score", "brightness_mean", "phash",
            "is_scene_boundary", "scene_index", "shot_type", "hdr_detected",
            "interlaced", "aspect_ratio", "extraction_method", "session_id",
        ]
        for key in required_keys:
            assert key in d, f"Missing metadata key: {key}"

    def test_interval_clamped_to_minimum_1(self):
        """interval_sec < 1 must be clamped to 1."""
        clamped = max(1, min(int(-5), 300))
        assert clamped == 1

    def test_interval_clamped_to_maximum_300(self):
        clamped = max(1, min(int(99999), 300))
        assert clamped == 300

    def test_resize_if_needed_reduces_oversized_frame(self):
        if cv2 is None:
            pytest.skip("cv2 not available")
        big = np.zeros((4096, 4096, 3), dtype=np.uint8)
        resized = _resize_if_needed(big, max_dim=1024)
        h, w = resized.shape[:2]
        assert max(h, w) <= 1024

    def test_resize_if_needed_noop_for_small_frame(self):
        if cv2 is None:
            pytest.skip("cv2 not available")
        small = np.zeros((100, 200, 3), dtype=np.uint8)
        result = _resize_if_needed(small, max_dim=1024)
        assert result.shape == small.shape

    def test_classify_shot_returns_valid_type(self):
        if cv2 is None:
            pytest.skip("cv2 not available")
        frame = np.zeros((360, 640, 3), dtype=np.uint8)
        shot  = _classify_shot(frame)
        assert shot in ("wide", "medium", "close", "unknown")

    def test_tonemap_frame_clips_to_uint8(self):
        if cv2 is None:
            pytest.skip("cv2 not available")
        hdr_frame = np.random.randint(0, 65535, (100, 100, 3), dtype=np.uint16)
        mapped    = _tonemap_frame(hdr_frame.astype(np.float32))
        assert mapped.dtype == np.uint8

    def test_generate_thumbnail_grid_creates_file(self, tmp_path):
        if cv2 is None:
            pytest.skip("cv2 not available")
        frames = []
        for i in range(3):
            img_path = str(tmp_path / f"f{i}.jpg")
            cv2.imwrite(img_path, np.zeros((180, 320, 3), dtype=np.uint8))
            meta = FrameMetadata(
                frame_index=i, timestamp_start=float(i), timestamp_end=float(i+1),
                path=img_path, frame_width=320, frame_height=180,
                fps=25.0, video_duration=10.0, video_id="test.mp4",
                blur_score=0.9, brightness_mean=0.0, phash=None,
                is_scene_boundary=False, scene_index=i, shot_type="wide",
                hdr_detected=False, interlaced=False, aspect_ratio=1.78,
                extraction_method="opencv_fallback", session_id="sess-001",
            )
            frames.append(meta)

        grid_path = str(tmp_path / "grid.jpg")
        result = generate_thumbnail_grid(frames, grid_path, rows=1, cols=3)
        assert result is not None
        assert Path(result).exists()

    def test_cleanup_removes_directory(self, tmp_path):
        d = tmp_path / "frames_test"
        d.mkdir()
        (d / "f.jpg").write_bytes(b"data")
        cleanup_frame_temp_dir(str(d))
        assert not d.exists()

    @pytest.mark.asyncio
    async def test_async_extract_raises_on_missing_file(self):
        with pytest.raises((FileNotFoundError, Exception)):
            await extract_frames_async("/nonexistent/video.mp4", "sess-001")

    def test_opencv_not_available_raises_import_error(self, monkeypatch):
        import app.ingestion.video_frames as vf
        original = vf.cv2
        vf.cv2 = None
        with pytest.raises((ImportError, Exception)):
            extract_frames("/tmp/test.mp4", 2, "sess-001")
        vf.cv2 = original

    def test_no_frames_extracted_error_subclass(self):
        assert issubclass(NoFramesExtractedError, VideoFrameError)
        assert issubclass(VideoOpenError, VideoFrameError)
        assert issubclass(DiskSpaceError, VideoFrameError)

    def test_frame_metadata_aspect_ratio_correct(self):
        meta = FrameMetadata(
            frame_index=0, timestamp_start=0.0, timestamp_end=1.0,
            path="/tmp/f.jpg", frame_width=1920, frame_height=1080,
            fps=30.0, video_duration=120.0, video_id="v.mp4",
            blur_score=0.8, brightness_mean=130.0, phash=None,
            is_scene_boundary=False, scene_index=0, shot_type="wide",
            hdr_detected=False, interlaced=False, aspect_ratio=round(1920/1080, 3),
            extraction_method="pyscenedetect", session_id="sess-001",
        )
        assert abs(meta.aspect_ratio - 1.778) < 0.01

    def test_detect_scenes_pyscenedetect_returns_list_on_failure(self):
        result = _detect_scenes_pyscenedetect("/nonexistent/video.mp4", 27.0)
